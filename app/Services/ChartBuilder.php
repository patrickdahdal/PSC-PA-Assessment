...
class ChartBuilder
{
    ...
    public static function buildChartImage(array $data)
    {
        ...
        // Traits labels alignment y-axis offsets in px
        $offset = [
        ...
        $xMinValue   = -100;
        $xMaxValue   = 100;
        $xAxisWidth = $xMaxValue - $xMinValue;
        $xAxisCorrection = ($xMinValue < 0 ? abs($xMinValue) : 0);
        ...
        for ($i = 0; $i <= $xAxisWidth; $i += 20) {
            $x = $gridLeft + $i * $gridWidth / $xAxisWidth;
    
            // Draw the grid line
            imageline($chart, $x, $gridTop, $x, $gridBottom, $gridColor);
    
            // Draw the x-axis label
            $xLabel = $i - $xAxisCorrection;
            $labelBox = imagettfbbox($fontSize, 0, $font, $xLabel);
            $labelWidth = $labelBox[4] - $labelBox[0];
    
            $labelX = $x - $labelWidth / 2;
            $labelY = $gridBottom + 30;
    
            imagettftext($chart, $fontSize, 0, $labelX, $labelY, $labelColor, $font, $xLabel);
        }
        ...
        $itemY = $gridTop + $barSpacing / 2;
    
        foreach ($data as $key => $score) {
            // Draw the bar
            $x1 = $gridLeft;
            $x2 = $gridLeft + ($score + $xAxisCorrection) / $xAxisWidth * $gridWidth;
            $y1 = $itemY - $barHeight / 2;
            $y2 = $itemY + $barHeight / 2;
    
            imagefilledrectangle($chart, $x1, $y1, $x2, $y2, $barColors[$key]);
    
            // Draw the label
            imagettftext($chart, $fontSize, 0, 20, $itemY + $fontSize / 2, $labelColor, $fontBig, $key);
    
            // Draw trait name and score above the bar
            imagettftext($chart, $fontSize, 0, $x2 + 10, $itemY + $fontSize / 2, $labelColor, $font, $score);
            imagettftext($chart, $fontSize, 0, 20, $itemY - 20, $labelColor, $font, $legend[$key]);
    ...
    public static function buildEmailChartImage(array $data)
    {
        // Validate chart data
        if (empty($data) || count($data) != 10) {
            return false;
